  ...
  if (!isAvailable) {
    return (
      <motion.article
        ...
        className="flex min-h-28 flex-col rounded-2xl border border-slate-100 bg-slate-50/70 p-3 ring-1 ring-slate-900/[0.03]"
        aria-label={`${edital.title} — em breve`}
      >
        <div className="mb-3 flex items-center justify-between gap-2">
          <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl opacity-70 ring-1 ${softIcon}`}>
            <Icon className="h-3.5 w-3.5" aria-hidden="true" />
          </span>
          <span className="inline-flex items-center gap-1 rounded-full bg-white px-2 py-1 text-[9px] font-bold leading-none text-slate-400 ring-1 ring-slate-200">
            <Clock className="h-2.5 w-2.5 shrink-0" aria-hidden="true" />
            Em breve
          </span>
        </div>
        <div className="min-w-0">
          <p className="line-clamp-2 text-xs font-bold leading-4 text-slate-500">
            {edital.title}
          </p>
