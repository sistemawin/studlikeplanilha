...
export function Exams({
  ...
  const hourGoal = goals.find((g) => g.tipo === "horas")!;
  const questionGoal = goals.find((g) => g.tipo === "questões")!;
  ...
  const bestTopics = topics
    .map((t) => {
      ...
      return {
        topic: t,
        score: topicScore(t.status, t.dificuldade),
        subject,
        accent: corToAccent(subject?.cor ?? ""),
      };
    })
  ...
  return (
    <>
      {/* Analytics charts */}
      <section
        className={`${isVisible ? "grid" : "hidden"} gap-6 lg:grid xl:grid-cols-[1fr_1fr]`}
      >
        <PieChart
      ...
      <section
        className={`${isVisible ? "grid" : "hidden"} gap-5 lg:grid xl:grid-cols-[1.1fr_0.9fr]`}
      >
        ...
        >
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div className="min-w-0 rounded-xl border border-blue-200 bg-blue-50 p-4">
              ...
              <ProgressBar
                value={pct(hourGoal.valorAtual, hourGoal.valorObjetivo)}
                tone="bg-blue-500"
            ...
            <div className="mt-3 space-y-2">
              {exams.map((exam) => (
                ...
                >
                  ...
                  <span className="shrink-0 text-sm font-semibold">{pct(exam.acertos, exam.total)}%</span>
                </div>
              ))}
            </div>
          </div>
